...
export class DeveloperPair {
    ...
    public calculate_compatibility() {
        ...
        const compatibility_func = (habilidad1: number, habilidad2: number ) =>  Math.min(habilidad1, habilidad2) * 100 / Math.max(habilidad1,
                                                                                            habilidad2);
        console.log("id_skills: ", id_skills);
        for (const id_habilidad in id_skills) {
